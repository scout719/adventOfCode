...
extractChecksum (x:xs) = 
    if x == ']' then 
       "" 
    else
        x:(extractChecksum xs)

extractRoomInfo :: String -> (String, String, String)
extractRoomInfo [] = ("","","")
extractRoomInfo (x:xs) = 
    if x == '[' then
        ("","",extractChecksum xs)
    else
...
solve1 ((x, id, checksum):xs) =
    let freq_map = sort (countOccurrences x [])
        rest = solve1 xs
    in if validateChecksum checksum freq_map then
        (read id) + rest
       else 
        rest 
